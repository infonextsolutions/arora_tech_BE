...
function secureInput($var)
{
	...
	if (is_array($var)) {
		...
		if (function_exists("get_magic_quotes_gpc")) {
			$output = mysqli_real_escape_string($GLOBALS['db'], get_magic_quotes_gpc() ? stripslashes($var) : $var);
		} else {
			$output = mysqli_real_escape_string($GLOBALS['db'], $var);
		}
	}
	if (!empty($output))
		return $output;
...
function login($user, $pass)
{
	...
	$query = mysqli_query($GLOBALS['db'], 'SELECT id, username, password, active, level_access FROM users WHERE username = "' . secureInput($user) . '" AND password = "' . secureInput($pass) . '" AND level_access != 9') or die(mysqli_error($GLOBALS['db']));

	if (mysqli_num_rows($query) == 1) {
		$row = mysqli_fetch_assoc($query);
		$update = mysqli_query($GLOBALS['db'], 'UPDATE users SET last_login = "' . $lastLogin . '" WHERE id = "' . $row['id'] . '"');
		if ($row['active'] == 1) {
			set_login_sessions($row['id'], $row['password'] ? TRUE : FALSE);
			if ($row['level_access'] == 2 || $row['level_access'] == 1) {
				return 99;
			}
		}
		if ($row['active'] == 2) {
...
function checkLogin($levels)
{
	...
	if (!$_SESSION['logged_in']) {
		...
		if (isset($_COOKIE['cookie_id'])) { //if we have a cookie
			...
			$query = mysqli_query($GLOBALS['db'], 'SELECT * FROM users WHERE id = "' . mysqli_real_escape_string($GLOBALS['db'], $_COOKIE['cookie_id']) . '"');
...
{
	$query = mysqli_query($GLOBALS['db'], "SELECT `level_access` FROM `users` WHERE `id` = '" . 	mysqli_real_escape_string($GLOBALS['db'], $user_id) . "'");
	if (mysqli_num_rows($query) == 1) {
		...
		$message = "
					<html>
					  ...
					  <body>
						...
						<p>Dear " . strtoupper($user) . ", thank you for registering at Arora Realtech.</p>
						<p>Please check with Arun Arora/Admin to activate your account:</p>
						<p>If you did not initiate this request, simply disregard this email, and we're sorry for bothering you.</p>
						<br/><br/>
						<p>Sincerely,</p>
						<p>The Arora RealTech Team.</p>
					  </body>
					</html>
					";
		// To send HTML mail, the Content-type header must be set
		$headers = "MIME-Version: 1.0\r\n";
		$headers .= "From: property.arorarealtech.in \r\n";
...
function editUser($id, $email, $firstname, $lastname, $dialing_code, $phone, $city, $country)
{
	...
	if (!empty($email)) {
		...
		$res = mysqli_query($GLOBALS['db'], $sql) or die(mysqli_error($GLOBALS['db']));
		if (!$res) return 4;
		return 99;
	}
	if (empty($email)) {
...
function updatePass($id, $opass, $pass)
{
	...
	$query = mysqli_query($GLOBALS['db'], 'SELECT `password` FROM `users` WHERE `id` = "' . $id . '"');
	$row = mysqli_fetch_assoc($query);
...
function deleteUser($id)
{
	...
	if ($res) {
		...
		$result = mysqli_query($GLOBALS['db'], $del);
		if ($result)
			return 99;
		return 1;
	} else return 2;
...
function getUserRecords($id)
{
	...
	$c = 0;
	while ($a_row = mysqli_fetch_array($res)) {
...
function adminLogin($user, $pass)
{
	...
	//Use the input username and password and check against 'users' table
	$query = mysqli_query($GLOBALS['db'], 'SELECT id, username, password, active, level_access FROM users WHERE username = "' . secureInput($user) . '" AND password = "' . secureInput($pass) . '" AND level_access = 1') or die(mysqli_error($GLOBALS['db']));
...
function pass_recovery($email, $site_url)
{
	...
	if ($num == 1) {
		...
		$update = mysqli_query($GLOBALS['db'], "UPDATE users SET password='" . $temp_pass . "',temp_pass='" . $temp_password . "',temp_pass_active=1 WHERE email='" . $email . "'") or die(mysqli_error($GLOBALS['db']));
...
function confirm_pass($id, $new)
{
	...
	$query = mysqli_query($GLOBALS['db'], "SELECT password,temp_pass,temp_pass_active FROM users WHERE id = '" . $id . "'");

	if (mysqli_num_rows($query) == 1) {
		$row = mysqli_fetch_assoc($query);
		if ($row['temp_pass'] == $_GET['new'] && $row['temp_pass_active'] == 1) {
			$update = mysqli_query($GLOBALS['db'], "UPDATE users SET temp_pass_active=0 WHERE id = '" . mysqli_real_escape_string($GLOBALS['db'], $_GET['id']) . "'");
			if ($update) {
				return 99;
			} else return 1;
		} else {
...
function confirm_user_reg($prsn)
{
	$prsn = secureInput($prsn);

	$query = mysqli_query($GLOBALS['db'], "SELECT username,active FROM users WHERE username = '" . $prsn . "'");

	if (mysqli_num_rows($query) == 1) {
		$row = mysqli_fetch_assoc($query);
		if ($row['active'] == 0) {
			$update = mysqli_query($GLOBALS['db'], "UPDATE users SET active=1 WHERE username = '" . $prsn . "'");
			if ($update) {
				return 99;
			} else return 1;
		}
...
function displayUserImg($id)
{
	...
	if (!empty($row['thumb_path'])) {
		echo "<img src='" . $row['thumb_path'] . "' width='250' height='200' border='0' alt='' hspace='2' />";
	} else {
