...
$id = $_GET['id'];
$result = mysqli_query($GLOBALS['db'], "Select * from builddeal where RID='$id'") or die(mysqli_error($GLOBALS['db']));
	...
	<form id="demoForm" method="POST" action="Updateflooralt.php" class="col-sm-6">
		<div id="fieldWrapper"><span class="step" id="first">
				...
				<table class="table">
					...
					<input type="hidden" name="table" id="name" value="builddeal" class="required" rel="0" />
					...
					<tr>
						...
						<td><INPUT TYPE="radio" NAME="buildup" VALUE="simplex" <?php if (strtolower($row['buildup']) == 'Simplex') echo 'checked'; ?>>Simplex<br />
		...
		<tr>
			...
			<td>
				<div class="highlight"><input class="input_field_25em" type="radio" name="file" value="Y" <?php if ($row['file'] == 'Y') echo 'checked'; ?>>Yes</>
					<input class="input_field_25em" type="radio" name="file" value="N" <?php if ($row['file'] == 'N' || $row['file'] == '-') echo 'checked'; ?>>No </>
			</td>
</div>
</tr>
<tr>
